...
PlyEncoderCustom::PlyEncoderCustom() : out_buffer_(nullptr), in_point_cloud_(nullptr), in_mesh_(nullptr) {};

bool PlyEncoderCustom::EncodeToFile(const draco::PointCloud &pc, const std::string &file_name) {
  std::unique_ptr<draco::FileWriterInterface> file = draco::FileWriterFactory::OpenWriter(file_name);
  if (!file) {
...
bool PlyEncoderCustom::EncodeToFileBuffer(const draco::Mesh &mesh, FILE *pFileBuffer)
{
	in_mesh_ = &mesh;
  // TODO in_mesh_ not being recognized correctly or something
  printf("DEBUG: in encode to file buffer set in_mesh to mesh\n");
	return EncodeToFileBuffer(static_cast<const draco::PointCloud &>(mesh), pFileBuffer);
}

bool PlyEncoderCustom::EncodeToFileBuffer(const draco::PointCloud &pc, FILE *pFileBuffer)
{
	
  if(in_mesh_ == nullptr){
    printf("DEBUG in encode to file buffer, in_mesh_ is NULL\n");
  }
	if (!pFileBuffer)
	{
    printf("pFileBuffer NULL - failed\n");
		return false; // File couldn't be opened.
	}
	// Encode the mesh into a buffer.
	draco::EncoderBuffer buffer;
	if (!EncodeToBuffer(pc, &buffer))
	{
    printf("EncodeToBuffer failed\n");
		return false;
	}
	// Write the buffer into the file.
  // TODO comment out two lines below this
  std::unique_ptr<draco::FileWriterInterface> file = draco::FileWriterFactory::OpenWriter("/home/allan/draco_encode_cpp/bun_zipper2.ply");
  file->Write(buffer.data(), buffer.size());
  // FILE* ptr = fopen("/home/allan/draco_encode_cpp/bun_zipper3.ply","w+");
  // WriteMeshIntoStream(mesh0.get(), ss, MESH_EDGEBREAKER_ENCODING);
  // draco::io::WriteMeshIntoStream();

	return (fwrite(buffer.data(), 1, buffer.size(), pFileBuffer) == buffer.size());
	
  // std::unique_ptr<draco::FileWriterInterface> file = draco::FileWriterFactory::OpenWriter("/home/allan/draco_encode_cpp/bun_zipper2.ply");
  // std::unique_ptr<draco::FileWriterInterface> file = draco::FileWriterFactory::OpenBuffer("/home/allan/draco_encode_cpp/bun_zipper2.ply");
	// return (file->Write(buffer.data(), buffer.size()) == buffer.size());
}

bool PlyEncoderCustom::EncodeToFile(const draco::Mesh &mesh, const std::string &file_name) {
  in_mesh_ = &mesh;
  return EncodeToFile(static_cast<const draco::PointCloud &>(mesh), file_name);
}

bool PlyEncoderCustom::EncodeToBuffer(const draco::PointCloud &pc, draco::EncoderBuffer *out_buffer) {
  in_point_cloud_ = &pc;
...
bool PlyEncoderCustom::EncodeToBuffer(const draco::Mesh &mesh, draco::EncoderBuffer *out_buffer) {
  in_mesh_ = &mesh;
  return EncodeToBuffer(static_cast<const draco::PointCloud &>(mesh), out_buffer);
}
bool PlyEncoderCustom::EncodeInternal() {
  // Write PLY header.
  ...
  const int pos_att_id =
      in_point_cloud_->GetNamedAttributeId(draco::GeometryAttribute::POSITION);
  int normal_att_id =
      in_point_cloud_->GetNamedAttributeId(draco::GeometryAttribute::NORMAL);
  int tex_coord_att_id =
      in_point_cloud_->GetNamedAttributeId(draco::GeometryAttribute::TEX_COORD);
  const int color_att_id =
      in_point_cloud_->GetNamedAttributeId(draco::GeometryAttribute::COLOR);
  ...
  for (draco::PointIndex v(0); v < num_points; ++v) {
  ...
  if (in_mesh_) {
    // Write face data.
    for (draco::FaceIndex i(0); i < in_mesh_->num_faces(); ++i) {
  ...
  printf("DEBUG: in ply encoder custom, just finished encoder internal successfully\n");
  return true;
}

bool PlyEncoderCustom::ExitAndCleanup(bool return_value) {
  in_mesh_ = nullptr;
...
const char *PlyEncoderCustom::GetAttributeDataType(int attribute) {
